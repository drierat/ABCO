...
%
nextMovement(NProb,[H|_],Chosen,Chosen):-(NProb =< H -> true).
nextMovement(NProb,[H|T],ChosenIn,Chosen):-(NProb>H -> NProb_1 is NProb-H,
...
MaxCap = 100,
% Notice that the first client (i.e. [86,22] corresponds to the depot
...
%substr(+X,+Y,-Z) removes elements in X from list Y to return list Z 
%
substr(X,Y,Z):-
...
best(LQuality,Best,L):-
	L is min(LQuality),
	write('The minimum cost in this iteration is '),writeln(L),			%TO REMOVE
	ith(Best,LQuality,L),!.
